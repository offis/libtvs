...
timed_base::sync()
{
  ...
  object_host::for_each_stream_in_scope([&max_time](timed_stream_base* stream) {
...
{
  object_host::for_each_stream_in_scope([&until](timed_stream_base* stream) {
    stream->commit(until);
    return false;
  });
  return until;
...
timed_base::sync_current_scope()
{
  ...
  object_host::for_each_stream_in_scope([&time](timed_stream_base* stream) {
    time = stream->sync();
    return true;
  });
